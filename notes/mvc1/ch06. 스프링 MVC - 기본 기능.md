...
- `@RequestMapping("/hello-basic")`
  ...
  - 대부분의 속성을 배열로 제공하므로 **다중 설정이 가능**하다. {"/hello-basic", "/hello-go"}

> 🍀 `/hello-basic`, `/hello-basic/` 두 요청은 다른 URL이지만, 스프링은 같은 요청으로 매핑한다.
...
```
- 만약, 해당 경로에 POST 매핑을 요청하면 스프링 MVC는 405 상태코드(Method Not Allowed)를 반환한다.
...
- @PathVariable 의 이름과 파라미터 이름이 같으면 생략할 수 있다. (예: @PathVariable("userId") String userId -> @PathVariable userId)
...
- `MultiValueMap`
  ...
  ```
### @Slf4j
다음 코드를 자동으로 생성해서 로그를 선언해 준다. 개발자는 편리하게 `log`로 사용하면 된다.
```java
private static fianl org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RequestHeaderController.class);
```

> 🍀 [스프링 MVC 공식 문서](https://docs.spring.io/spring-framework/reference/web/webmvc/mvc-controller.html)
...
1. GET, 쿼리 파라미터 전송 예시: `http://localhost:8080/request-param?username=hello&age=20`
...
GET 쿼리 파라미터 방식, POST HTML Form 전송 방식 모두 형식이 같기 때문에 조회할 수 있다.
...
```
- `request.getParameter`: 단순히 HttpServletRequest가 제공하는 방식으로 요청 파라미터를 조회했다.
...
@RequestParam의 `name(value)` 속성이 파라미터 이름으로 사용
  - @RequestParam("username") String memberName    
➡️ request.getParameter("username")
...
```
HTTP 파라미터 이름이 변수 이름과 같으면 `@Requestparam(name="xx")`을 생략 할 수 있다.
#### requestParamV4()
```java
@ResponseBody
...
String, int 등 단순 타입이면 `@RequestParam` 도 생략할 수 있다.  
🍀 애노테이션을 완전히 생략해도 되지만, `@RequestParam`이 있으면 명확하게 요청 파라미터에서 데이터를 읽는 다는 것을 알릴 수 있다.
...
- `@RequiredParam.required`
  - 파라미터 필수 여부
  - 기본 값이 파라미터 필수(ture)이다.
- `/request-param` 요청
  - `username` 이 없으므로 400 예외가 발생한다.

> 🚨 주의  
> 1. 파라미터 이름만 사용  
>    - `request-param?username=` 파라미터 이름만 있고 값이 없는 경우 ➡️ 빈문자로 통과된다.
> 2. 기본(primitive)에 null 입력
>    - `request-param` 요청 `@RequestParam(required = false) int age`에서 `null`을 `int`에 입력하는 것은 불가능하기에
> (500 예외 발생) `null`을 받을 수 있는 `Integer`로 변경하거나, 또는 `defaultValue`를 사용해야 한다.
...
`defaultValue`는 빈 문자의 경우에도 설정한 기본 값이 적용된다. `/required-param?username=`
...
파라미터의 값이 1개가 확실하다면 `Map`을 사용해도 되지만, 확실하지 않다면 `MutliValueMap`을 사용하자.
...
- `HelloData` 객체를 생성한다.
- 요청 파라미터의 이름으로 `HelloData` 객체의 파라미터를 찾는다. 
- 해당 프로퍼티의 `setter`를 호출해서 파라미터의 값을 바인딩한다. (예: 파라미터 이름이 `username`이라면, `setUsername()` 메서드를 찾아서 호출하면서 값을 바인딩한다.)
- 프로퍼티 객체에 `getUsername()`, `setUsername()` 메서드가 있으면, 이 객체는 `username` 이라는 프로퍼티를 가지고 있다.
- username 프로퍼티의 값을 변경하면 `setUsername()`이 호출되고, 조회하면 `getUsername()`이 호출된다.0
- model.addAttribute(helloData) 코도 함께 적용된다.

> 🍀 바인딩 오류: `age=abc` 처럼 숫자가 들어가야할 곳에 문자를 넣으면 `BindException`이 발생한다.
...
- HttpEntity: HTTP header, body 정보를 편리하게 조회
  - 메시지 바디 정보를 직접 조회한다.
  - 요청 파라미터를 조회하는 기능과 관계가 없다. (`@RequestParam` X, `@ModelAttribute` X)
- HttpEntity는 응답에서 사용 가능하다.
  - 메시지 바디 정보 직접 반환
  - 헤더 정보 포함 가능
  - view 조회 X
...
```
- HttpServletRequest를 사용해서 직접 HTTP 메시지 바디에서 데이터를 읽어와서, 문자로 반환한다.
...
- HttpMessageConverter 사용 ➡️ StringHttpMessageConverter 적용
- **문자로 변환하고 다시 josn으로 변환하는 과정이 불편한다.**
...
- HttpMessageConverter사용 ➡️ MappingJackson2HttpMessageConverter (content-type: application/json)
`HttpEntity`, `@RequestBody`를 사용하면 HTTP 메시지 컨버터가 HTTP 메시지 바디의 내용을 우리가 원하는 문자나 객체 등으로 변환해 준다.  
HTTP 메시지 컨버터는 문자 뿐만 아니라 JSON도 객체로 변환해 주는데, V2에서 했던 작업을 직접 처리해준다.
- 생략 불가
  - 이 경우에 HelloData 에 `@RequestBody`를 생략하면, `@ModelAttribute`가 적용된다.
  - 즉, HTTP 메시지 바디가 아니라 쵸어 파라미터를 처리하게 된다.
...
- `src/main/resources`는 리소스를 보관하는 곳이고, 클래스 패스의 시작 경로이다. 따라서 다음 디렉토리에 리소스를 넣어두면 스프링 부트가 정적 리소스로 서비스를 제공한다.
- 경로: `src/main/resoucres/static`
- 다음 경로의 파일이 있다면, 웹 브라우저에서 다음과 같이 실행하면 된다.정적 리소스는 해당 파일을 변경없이 그대로 서비스하는 것이다.
...
스프링 부트는 뷰 템플릿 경로를 제공한다. (`src/main/resources/templates`)
...
- `response.getWriter().write("ok")`
#### responseBodyV2()
- `ResponseEntity`는 `HttpEntity`를 상속 받았는데, `HttpEntity`는 HTTP 메시지의 헤더, 바디 정보를 가지고 있다.
...
- HTTP 메시지 컨버터를 통해서 JSON 형식으로 변환되어 반환한다.
#### responseBodyJsonV2()
- `ResponseEntity`는 HTTP 응답 코드를 설정할 수 있는데, `@ResponseBody`를 사용하면 이런 것을 설정하기 까다롭다.
- `@ResponseStatus(HttpStatus.OK)` 애노테이션을 사용하면 응답 코드로 설정할 수 있다.
- 물론 애노테이션이기 때문에 응답 코드를 동작으로 변경할 수 없다. 프로그램 조건에 따라서 동적으로 변경하려면 `ResponeBody`를 사용하면 된다.
#### RestController
- `@Controller` 대신 `@RestController` 애노테이션을 사용하면, 해당 컨트롤러의 모든 메소드에 `@ResponseBody`가 적용되는 효과가 있다.
- 따라서 뷰 템플릿을 사용하는 것이 아니라, HTTP 메시지 바디에 직접 데이터를 입력한다.
- 이름 그대로 Rest API (HTTP API)를 만들 때 사용하는 컨트롤러이다.
- 참고로 `@ResponseBody`는 클래스 레벨에 두면 전체 메서드에 적용되는데, `@RestController` 애노테이션 안에 `@ResponseBody`가 적용되어 있다.

## HTTP 메시지 컨버터
뷰 템플릿으로 HTML 을 생성해서 응답하는 것이 아니라, HTTP API 처럼, JSON 데이터를 HTTP 메시지 바디에 직접 읽거나 쓰는 경우 HTTP 메시지 컨버터를 사용하면 편리하다.

### @ResponseBody 사용 원리
![ResponseBody 원리.png](imgs%2FResponseBody%20%EC%9B%90%EB%A6%AC.png)
#### `@ResponseBody` 사용
- HTTP의 BODY에 문자 내용을 직접 반환
- `viewResolver` 대신에 `HttpMessageConverter`가 동작
- 기본 문자처리: `StringHttpMessageConveter`
- 기본 객체처리: `MappingJackson2HttpMessageConveter`
- byte 처리 등 기타 여러 HttpMessageConveter가 기본으로 등록되어 있다.

> 🍀 응답의 경우 클라이언트의 HTTP Accept 헤더와 서버의 컨트롤러 반환 타입 정보 둘을 조합해서 `HttpMessageConveter`가 선택된다.
...
### HTTP 메시지 컨버터 인터페이스
`org.springframework.http.conveter.HttpMessageConveter`
```java
...
2. 메시지 컨버터가 메시지를 쓸 수 있는지 확인하기 위해 `canWrite()`를 호출한다.
   ...
   - HTTP 요청의 Accept 미디어 타입을 지원하는가? (더 정확히는 `RequestMapping`의 `produces`)
     - 예) `text/plain`, `application/json`, `*/*`
3. `canWrite()` 조건을 만족하면 `write()` 호출해서 HTTP 응답 메시지 바디에 데이터를 생성한다.

## 요청 매핑 핸들러 어댑터 구조
![스프링 MVC 구조.png](imgs%2F%EC%8A%A4%ED%94%84%EB%A7%81%20MVC%20%EA%B5%AC%EC%A1%B0.png)
HTTP 메시지 컨버터는 스프링 MVC 어디쯤에서 사용될까? 모든 비밀은 애노테이션기반의 컨트롤러 즉, `@RequestMapping`을 처리하는 핸들러 어댑터
`RequestMappingHandlerAdapter` (요청 매핑 핸들러 어댑터)에 있다.

### RequestMappingHandlerAdapter 동작 방식
![RequestMappingHandlerAdatper 동작 방식.png](imgs%2FRequestMappingHandlerAdatper%20%EB%8F%99%EC%9E%91%20%EB%B0%A9%EC%8B%9D.png)

#### ArgumentResolver
애노테이션 기반의 컨트롤러는 매우 다양한 파라미터를 사용할 수 있다.
`HttpServletRequest`, `Model`, `@RequestParam`, `@ModalAttribute`, `@RequestBody`, `HttpEntity` 등등 이렇게 파라미터를 유연하게
처리할 수 있는 이유가 바로 `ArgumentResolver` 덕분이다.

애노테이션 기반 컨트롤러를 처리하는 `RequestMappingHandlerAdapter`는 바로 이 `ArgumentResolver`를 호출해서 컨트롤러가 필요로하는
다양한 파라미터의 값 (객체)를 생성한다. 그리고 이렇게 파라미터의 값이 모두 준비되면 컨트롤러를 호출해서 값을 넘겨준다.

스프링은 30개 가 넘은 `ArugmentResolver`를 기본으로 제공한다. 

> 🍀 [스프링 공식 문서](https://docs.spring.io/spring-framework/reference/web/webmvc/mvc-controller/ann-methods/arguments.html)

정확히는 `HandleMethodArguemntResolver` 인데 줄여서 `ArgumentResolver`라고 부른다.
```java
...
> 🍀  [스프링 공식 문서](https://docs.spring.io/spring-framework/reference/web/webmvc/mvc-controller/ann-methods/return-types.html)
...
> 🍀 `HttpMessageConveter`를 구현한 클래스를 한 번 확인해 보자.
...
실제 자주 사용하진 않으니 실제 기능 확장이 필요할 때 `WebMvcConfigurer`를 검색하자.
