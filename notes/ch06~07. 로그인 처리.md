...
### LoginCheckFilter.java 생성 (인증 체크 필터)
```java
@Slf4j
...
### `Webconfig.java`에 loginCheckFilter() 추가
```java
@Bean
...
#### `LoginController.java`의 login() 수정
```java
@PostMapping("/login")
...
## 스프링 인터셉터 요청 로그
### `LogInterceptor.java` 생성 (요청 로그 인터셉터)
```java
@Slf4j
